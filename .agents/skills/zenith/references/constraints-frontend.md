...
| 同域子路径部署的浏览器存储 | 入口 `polyfills.ts` 安装的 deployment-scoped Storage facade；构建配置使用唯一 `VITE_DEPLOYMENT_ID`，详细约定见 [browser-storage-isolation.md](../../../../docs/guide/browser-storage-isolation.md) | 直接调用 `localStorage.clear()`；新增未经过部署 namespace 的全局 storage / BroadcastChannel / Cache Storage key；用 `VITE_BASE_URL` 或 analytics `appId` 替代 deployment ID | 多个派生项目在同一 Origin 下串登录态、草稿、埋点队列或 PWA 缓存 |
...
- **标签溢出列**：表格中需要单行展示多枚标签、超出部分收纳为 `+N` 并通过 Popover 查看时，一律使用
  ...
  `{ key, label }`（配色由业务数据逐项决定时再带上 `color`，缺省回退列级 `tagColor`），并按业务传入 `tagColor` / `tagSize` / `popoverWidth` / `popoverTrigger` / `empty`。
  收起方式默认按列宽自适应（标签数量与文案长度由数据决定时用）；标签集合固定、文案长度相近（如订阅事件 / Scope）
