...
| 外部作用域切换回第 1 页 | `useListSearch` / `usePagination` 的 `resetKey`（当前公众号 / 站点 / 空间 / 目录…；多个来源传数组 `resetKey: [spaceId, folderId]`，按元素比较）：作用域变化时在同一渲染内回到第 1 页、筛选条件保留 | `useEffect(() => setPage(1), [scopeId])`（ESLint 已封禁）；在切换控件的 `onChange` 里手写 `setPage(1)` 属合法但不必要 | effect 版会先用旧页码请求一次新作用域的数据；各页漏写则切换后停在越界空页 |
...
- **状态列**：启停开关列一律 `useStatusToggle(...).column()`（默认「状态」/ 80 / `fixed: 'right'`），只读状态用 `renderEnabledStatusTag` / `DictTag`
  ...
  加 `eslint-disable-next-line no-restricted-syntax -- 理由`）；
