...
- Provider 在 `descriptor.filters` 声明实际支持的字段，用 `relationFilterWhere` 把过滤下推到权限 SQL 的 LIMIT 之前；多态关系按有界扫描继续分页。游标认证范围必须包含规范化筛选；返回被跳过目标的继续位置时保留加密，不能以可读编码暴露隐藏对象标识。不能用筛选结果清空原始分组状态，也不能将仍有继续游标的空页视为没有数据。
- 分组筛选复用 `useListSearch`，以对象与分组隔离 query key，保留导航返回时的草稿和已提交条件。通过 `api()` 传递查询布尔值，序列化交给契约请求层。
- `item.action` 只提供经权限和状态判定的业务处理导航。列表与详情复用原模块动作，不能在关联模块新建审批、退款或重试写接口；只读模拟登录不显示处理动作。
- 详情路由维护在 Shared 的 `entity-detail-routes.ts`，Web 与通知共用；新增业务附件必须区分托管文件 UUID 与网盘节点 ID，并在内容下载时再次授权。
- 人工关联类型与正反语义由 `manual-relations.ts` 定义，解除必须保留类型和方向；不要把人工说明变成自动业务关系的事实来源。
- 对象关注按[对象关注与业务变化提醒](../../../../docs/guide/entity-watches.md)接入：可靠事件同业务事务提交，发送前重新验证当前权限及关注状态，并以当前授权对象重新渲染全部通知内容。没有真实事件或精确详情入口时不开放关注。
