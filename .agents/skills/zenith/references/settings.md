...
> 这类值一律进 `packages/shared/src/settings/` 的模块注册表；硬约束见
> [constraints.md → Shared 层](./constraints.md#shared-层step-3-4) / [Service 层](./constraints.md#service-层step-5) /
> [Route 层](./constraints.md#route-层step-6-7) 与 [constraints-frontend.md](./constraints-frontend.md)，
...
| 问题 | 结论 |
| --- | --- |
| 归哪个模块？ | 模块清单以 `registry.ts` 的 `SETTINGS_MODULES` 为准。能归入现有模块就加字段；只有全新业务域才建新模块 |
...
1. 在 `packages/shared/src/settings/modules/{module}.ts` 的 schema 上加叶子：
   ...
   - 字符串 / 数组叶子字段名不得含 `password` / `secret` / `token` / `apiKey` / `credential`
     （设置文档进审计快照与前端缓存，密钥走各自的加密存储；`validateSettingsRegistry()` 校验）
   - 通用设置页按类型自动渲染：boolean → 开关、number → 数字输入（min / max / 整数取自 schema）、enum → 下拉、
     string → 输入框、string[] → 标签输入、嵌套对象 → 分组；标签与说明取自 `.meta({ title, description })`。
     枚举展示文案在 `SettingsPage.tsx` 的 `ENUM_LABELS` 登记
...
5. 声明了 `page` 的模块在其专用页面补一行 `SettingRow`；其余模块页面零改动
...
- 读取规则（事务内禁读、不 `??` 默认值、不自建缓存 / 环境变量兜底）见 [constraints.md → Service 层](./constraints.md#service-层step-5)；
  事务场景先在 `db.transaction()` 外读取，再以参数传给事务函数（见 `drive-upload.service.ts` 的 `settings` 参数）
...
- [ ] 服务端读取符合 [constraints.md → Service 层](./constraints.md#service-层step-5)（不在事务内、无 `??` 默认值、无环境变量兜底）
