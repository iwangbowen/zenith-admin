...
> **触发场景**：业务功能包含大数据量、长耗时、需要进度 / 取消 / 重试的操作——
> Excel 导入、报表生成、数据迁移、消息群发、第三方同步等无法在正常 HTTP 请求窗口内可靠完成的工作。
> 接入边界（同步 `/batch` vs 任务中心、禁止自建轮询）见 [constraints.md → 异步任务](./constraints.md#异步任务)。
>
