...
- 存储路径：**S3（含 MinIO / Cloudflare R2 等 S3 兼容）已接入云原生 multipart**——分片经 `UploadPart` 直传云端、记录 ETag，`complete` 时 `CompleteMultipartUpload` 合并，服务器不暂存整文件；其余 provider（oss / cos / obs / azure / kodo / bos / local / sftp）走本地临时目录暂存 + 流式合并。驱动抽象 `MultipartDriver`（`packages/server/src/lib/file-storage.ts`）可按接口逐个补全其它云的原生 multipart。
