...
- **public**：拼接永久公开直链（`publicBaseUrl` 优先，否则按 provider 默认公网域名），浏览器直连对象存储。有对象级 ACL 的云（oss/s3/cos/obs/bos）要求文件上传时的 ACL 快照为 `public-read*`，否则自动降级 presigned；无对象级 ACL 的 provider（kodo/azure/local/sftp）信任管理员配置。
- **presigned**：用各云官方 SDK 本地签发限时下载直链（纯本地 HMAC 计算，不发网络请求），适合私有文件。local/sftp 不支持，自动降级 proxy；签名失败（配置不完整/SDK 异常）也自动降级 proxy 并记录告警日志。

**URL 字段合同**：列表/详情/附件 DTO 中 `url` **永远是稳定代理路径** `/api/files/{id}/content`（可持久化进业务数据，永不失效）；public 策略额外返回 `directUrl`（永久公开直链，仅渲染用，**禁止持久化**——换域名/换策略后会失效）。presigned 签名 URL 永不出现在列表响应中。
