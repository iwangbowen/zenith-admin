...
- **操作系统口径**：Win11 的 UA 冻结为 `Windows NT 10.0`，纯 UA 解析只能判到 Windows 10。展示面（登录日志 / 会话 / 操作日志的 OS 列）优先级为：请求头 `X-Zenith-Os`（前端经 Client Hints 算好后由请求层逐请求携带）＞ 登录请求体自报 `os`（首访覆盖）＞ 服务端 UA + `Sec-CH-UA-Platform-Version` 解析。原始 UA 另存（登录日志 `userAgent` / 操作日志 `userAgent` 列），自报值仅展示、不做风控依据；refresh 轮换与租户切换沿用原会话终端信息，不重解析。
