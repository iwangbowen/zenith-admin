...
- **操作系统口径**：Win11 的 UA 冻结为 `Windows NT 10.0`，纯 UA 解析只能判到 Windows 10。展示面（登录日志 / 会话 OS）优先用登录请求体自报的 `os`（前端经 Client Hints 精确到 Win11，拿不到时缺省、服务端回退 UA 解析）；审计面（操作日志 OS 列）只信服务端请求头（UA + 下发的 `Accept-CH: Sec-CH-UA-Platform-Version` 带来的 `Sec-CH-UA-Platform-Version`），不采纳自报值。自报可伪造，不做风控依据；refresh 轮换与租户切换沿用原会话终端信息，不重解析。
