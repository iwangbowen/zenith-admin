...
入站消息由公开回调落库，依赖 `(account_id, msg_id)` 部分唯一索引原子去重（微信重试时不写重复记录）。消息列表支持按 openid、方向、类型、关键词筛选；会话视图按 openid 聚合，取每个会话最后一条消息 + 消息总数 + 粉丝资料。
...
> 自动回复匹配在回调中执行（`resolveAutoReply`）：关注事件 → 关注回复；文本消息 → 关键词回复（按优先级 + 匹配方式）→ 未命中则记录热词并回默认回复。被动回复 XML 在安全模式下会 AES 加密返回。
