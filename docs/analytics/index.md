...
| 文档 | 内容 |
|------|------|
| [埋点采集 SDK](./tracking) | 自动采集（页面/点击/性能/API）、手动埋点 API、声明式 `data-track`、远程配置、离线重试、隐私合规 |
| [行为分析](./behavior) | 概览 KPI、趋势、实时、会话、漏斗、留存、路径、用户时间线、维度分布、Web Vitals、热力图 |
...
- **多租户隔离**：所有数据按 `tenantId` 隔离；采集端点支持匿名上报（登录前埋点），错误指纹含租户因子保证分组全局唯一。
