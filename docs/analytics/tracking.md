...
| 元素点击 | 全局捕获 `button` / `a` / `[role=button]` / `[data-track]` 点击，上报 `$autocapture`（按钮文案作为 `elementLabel`） |
| Web Vitals | 自动采集 `LCP` / `INP` / `CLS` / `FCP` / `TTFB`，上报为 `perf` 事件 |
...
- `trackPageviews` / `trackClicks` / `trackPerformance` / `trackApi`：分项开关
