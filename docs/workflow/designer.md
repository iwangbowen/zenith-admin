...
配置用户发起申请时填写的表单。设计器在此步顶部提供**表单类型**切换，支持三种方式：

- **表单库设计器**：从「表单库」选择一个已有可视化表单进行绑定复用，并提供表单预览。
- **自定义业务表单**：绑定你在 `src/pages` 下自行实现的 React 页面承载发起填写与查看，数据存入流程实例的 `formData`。
- **业务系统主导**：由业务模块（自有实体表/列表页）发起并关联流程，仅配置审批查看组件；业务数据留在业务模块自己的表。

详见 [表单设计](./form-design.md) 与 [业务模块接入工作流](./business-integration.md)。
