...
| 移动审批轻页 | `/approval.html`（独立 SPA 入口，头像菜单 → 移动审批） | 移动优先底部图标标签栏（待办/已办/我的申请/抄送我，主流 App TabBar，图标+文字+角标）、搜索、极速同意、下拉刷新、审批详情（进度提示、表单权限过滤、可编辑字段、签名、快捷短语、底部操作抽屉）、同意/驳回/转办、发起人撤回/催办、沟通评论、发起申请（搜索+最近使用）、待办批量同意（勾选/全选/不可批量项置灰）、连续审批（提交后自动进入下一条待办）、详情发起人头像块、抄送一键全部已读、页面切换过渡动画；与后台共享登录态，发起/审批中的自选审批人与转办均内置移动端人员选择器（搜索+头像多选底部抽屉），业务自定义表单场景引导回桌面端 |
