...
| 格式化控件 | 手机号、邮箱、身份证、网址、密码、验证码、评分、NPS 量表、公式 |
| 系统组件 | 人员选择、部门选择、字典选择、关联审批单 |
| 高级控件 | 附件、图片、省市区、手写签名、富文本、明细 |
| 布局控件 | 说明文字、流水号、分栏、分割线、分组、标签页、分步 |

字段可配置必填、默认值、默认值公式、占位提示、帮助文本、长度、正则、数值范围、日期限制、文件限制、选项颜色、动态可见、动态必填、动态只读、字段比较、自定义校验公式、明细列唯一和公式表达式。

### 设计器操作

- **画布交互**：字段支持右键菜单（上移/下移/复制/粘贴/创建副本/必填切换/存为模板/删除）与键盘操作（`Ctrl+C/V` 复制粘贴、`Delete` 删除、`↑/↓` 切换选中、`Esc` 取消选中、`Ctrl+Z/Shift+Z` 撤销重做）。
...
- **选项批量编辑**：选项类字段支持切换批量文本模式，多行粘贴（`值|显示名`）快速录入。
- **明细表**：填写时支持行复制与「从剪贴板粘贴」（Excel 多行粘贴自动按列解析、数字类型自动转换）；子列可配置固定列宽。
