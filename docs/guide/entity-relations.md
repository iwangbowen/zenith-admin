...
关系项可携带受控的 `origin` 摘要：`kind` 区分直接业务关联、业务规则推导、流程或事件触发、操作活动来源；已登记的安全事件标识通过 `eventType` 返回，不携带事件 payload。前端在分组和关系项旁以图标提供来源 Tooltip，`occurredAt` 继续作为关联时间摘要展示。来源说明沿用关系 Provider 的权限和数据范围，不通过摘要暴露数量或隐藏对象。
