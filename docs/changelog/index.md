...
- 登录失败不再锁定账号：失败按「账号 × 来源 IP」计数（`identitySecurity.loginChallenge`，默认单来源 20 次 / 多来源 5 个 IP、窗口 30 分钟），达阈值只要求验证码（响应与 MFA 挑战同构，前端展示验证码后重新提交），真正的用户凭正确密码 + 验证码始终能登录——旧实现下任何知道用户名的人都能用失败请求把真正的用户关在门外。用户列表改为显示「需验证码」并可一键清除（`POST /api/users/{id}/unlock`），会员登录同步改造；企业 LDAP 无验证码环节，只节流失败的那个来源。登录验证码答案改存 Redis（5 分钟 TTL、`GETDEL` 一次性消费），取码与验码不再要求同一节点。
