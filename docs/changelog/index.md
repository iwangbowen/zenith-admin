...
- 文件夹树支持行内「更多」操作：每个文件夹节点都能就地新建子文件夹、重命名、删除，不必先点选该目录（点选会连带切换右侧素材范围）；系统节点（全部素材 / 类型筛选项 / 根目录）不显示操作入口。
- 目录树节点按语义区分图标：文件夹、根目录与「全部素材」下的类型筛选节点各有独立图标与配色，类型节点可直接点进按类型筛选。

#### 内容管理

- 内容列表标题列的标记可直接筛选：列头按钮勾选置顶 / 推荐 / 热门 / 原创，多选按 AND 走服务端查询，并补齐原创的列表条件与导出口径。
- 内容日历改为月视图：格子显示当日事件件数，悬浮列出当天明细并可点进编辑页；按月一次取回（`GET /api/cms/contents/calendar`），不翻页。

#### 数据看板

- 看板图形丰富化：新增渠道分布图与内容形态环形图，发布趋势改为统一的 VChart 柱状图并按看板惯例拆懒加载 chunk；待办条集中展示编辑待办，热文 TOP10 补上发布时间与互动数并下移页底。
- 看板可穿透：统计卡与渠道分布图按所点口径跳转到对应内容列表或评论列表。

#### 界面偏好与标签栏

- 新增标签栏自动隐藏偏好：收起后鼠标悬停顶部即展开。
- 标签切换器改 Chrome 风格，支持最近关闭列表与快捷键操作。

### Changed

- CMS 标签管理支持按分组关键字搜索，站点选择移到搜索栏首位。
- 内容编辑页收紧页头标签栏与正文区块的纵向间距；图集行去卡片化改为分隔线并显示序号。
- 全站日历的月份切换统一改用月份选择器（会员签到记录 / 会员签到页 / 内容日历共用 `MonthCalendar`），周起始日跟随「一周起始日」偏好。
- 顶栏高度由 40px 收至 34px；自动隐藏标签栏的把手改用主题色。
- 主从布局折叠态的展开按钮加周期性舒缓提示动画，通知红点贴到图标右上角。

### Fixed

- 素材目录树改受控展开态：增删文件夹后保留仍然存在的展开项，并自动展开新增目录的祖先链使其可见。
- 内容日历此前看不到直接发布的内容：事件口径补上实际发布时间（`published_at`），并新增路由注册顺序守卫，避免新接口被先注册的 `GET /{id}` 吞掉。
- 办理历史的时间戳改用 `DateTimeText`，跟随「时间显示方式」偏好并内建空值占位。
- 修复两处测试与实现漂移：看板用例的 mock 未落在钩子真正被 import 的模块（无 Provider 时以「No QueryClient set」整体失败），标签切换器的相对时间断言与真实时钟耦合（满载时抖动）。
...
- 告警事件操作列新增「处理详情」，已认领 / 已处理的事件可查看处理备注。
- 会话回放操作列新增单行删除，时长列加宽。

### Fixed

- 内容编辑页保存中不再锁定富文本编辑器，修复编辑区闪烁。
- 修复 CMS 发布构建可能停滞的问题。
- 热力图密度色固定为蓝色系，不再跟随主题色。

### Changed

- 内容管理统计卡改为单行文案汇总，释放纵向空间。
- 链路追踪应用日志面板默认展开。
- 内容列表发布按钮移入行内，提交审核收进更多菜单。
- 任务项明细标识名称列加宽并单行省略。
...
- 站点可视化配置与内部导航补全，默认门户布局与导航升级；站点保存、删除、开通统计或继承后自动刷新树视图与列表。
- 站点主题色改用 ColorPicker 组件选择；未设置时显示空态触发块，不再回落内置默认色。
- 站点组成蓝图与编辑协作操作；已保存工作稿预览与发布审阅统一到同一条链路；站点媒体选择接入编辑器，切换字段不丢草稿。

#### 素材中心与选择器

- 素材中心拆分为「素材列表」与「素材治理」两个页签，关注面更聚焦。
- 素材列表预览列支持点击预览：图片进图集、文档进预览弹窗，与文件列表页共用同一套受保护文件预览设施。
- 素材库弹窗改为文件列表页同款网格卡片：真实缩略图优先（缺失时回退类型图标），复选框多选跨翻页累计，一次批量加入图集。
- 迷你分页支持悬停快速切页；紧凑 / 桌面两套分页默认值收敛为共享预设，表格与列表分页统一管理。

#### 登录与安全

- 登录失败不再锁定账号，改为按来源逐步升级验证码；单来源触发阈值放宽到 30 次。
- 登录失败突增实时告警，攻击进行中即可看到。

#### 其他

- 友情链接新增分组视图。
- 内容模型字段编辑改用侧边抽屉，字段行布局与对齐重排。
- 任务托盘：计数角标收回顶栏内、列表不再横向滚动、去重待处理标签，改用 Semi List 渲染。

### Fixed

- 列表表格在后台刷新（如批量上传逐个落库）时不再整表压暗闪烁，加载蒙层只在首载出现。
- 九处手写上下页按钮收敛为共享迷你分页，单页自动隐藏；SQL 控制台翻页期间置灰防错乱。
- 互动问卷、广告、分发等多处列表列宽与省略修复。
- 恢复移动端搜索与公开导航韧性，补齐数据库与浏览器回归缺口。
- 任务分发稳定化，发布与导入链路整合；最终校验错误清理。
- 登录日志移除口径不一致的活跃用户统计。

### Changed

- 移除验证码清理定时任务与处理器，验证码随验证失败自然过期。
...
- 内容保存不再直接改写公开正文：每次落库生成不可变修订（`checkpoint` / `submission` / `publication` / `restore` / `preview`），字段、关系与素材版本一并冻结；交付链路只读修订，读不到之后被编辑过的工作稿。
- 公开指针与工作稿解耦：回滚或再次发布后，编辑者手上更新的工作稿继续保留并标记「有未发布修改」；编辑状态按新公开修订重算一致性（发布后一致记 `clean`，不一致回到 `draft`），待审与审核决定不因发布被改写。
- 正文升级为结构化文档（`CmsBodyDocument`，`schemaVersion: 1`，受限标签白名单），随修订冻结，编辑、渲染与检索共用同一份节点树。
- 写入继续以 CAS 版本保护：单条变更携带 `expectedVersion`，批量变更携带逐条 `expectedVersions`（缺项直接返回 400），版本不匹配返回冲突而不是静默覆盖。
- 内容模型也有版本：模型定义发布后固定为 `cms_model_versions`，内容行记录 `modelVersionId`，字段改名或改类型不会追溯改写已发布内容的展示口径。

#### 编辑协作

- 审稿批注：可挂在整篇内容、指定字段或某一修订上，支持 @ 提及、处理与重新打开；内容列表顶部指标新增「待处理批注」。
- 人工语言变体：独立创建、独立审核发布的翻译稿，源稿变更后标记「源稿已变化」，由编辑决定是否重译。
- 内容类型转换：提交前先预览字段损失与质量问题，确认后用显式字段映射生成转换工作稿，原条目不被就地改写。
- 分发与映射冲突：源、目标库与传入值三方冲突时逐字段选择保留哪一侧，选择结果合并为工作稿（`distribution-merge`）。
- 结构化字段质量检查（`validateCmsStructuredFields`）统一输出 `CmsQualityIssue`：字段配置校验、类型转换预览与内容质量检查共用同一套问题模型。

#### 发布单与公开代次

- 发布单固定已批准的修订与撤下清单，并记录创建时的 `baseGenerationId`；构建成功后才允许激活——「已提交」不再等于「已上线」。状态机为 `draft / building / ready / scheduled / active / failed / cancelled / superseded`，部署状态 `building / ready / active / retired / failed`。
- 构建由任务中心 `cms-release-build` 执行：worker 在 PostgreSQL 中创建以服务器数字 ID 命名的 `cms_generation_{id}` 关系快照，复制本站公开定义并在快照上应用目标修订；编辑、权限、会话、评论、互动答卷、访问统计与素材授权不入快照。
- 静态产物写入 `CMS_STATIC_ROOT/{siteCode}/generation-{id}/`，定义缓存按代次隔离或绕过；候选渲染不写公开缓存、访问统计与 CDN。生成后记录关系快照摘要与文件 SHA-256 清单。
- 激活前校验产物完整性、精确修订批准与素材授权，在站点锁内比较当前代次与预期代次（`expectedGenerationId`）；任何失败都保留旧代次，并发发布冲突返回 409。每次真正的激活与回滚追加 `cms_release_activations` 记录。
- 候选预览 `GET /api/cms/releases/{id}/preview?path=/...` 按固定候选代次返回 HTML 并内联候选主题 CSS，不加载脚本、不写访问统计，用于逐路径审阅。
- 公开读取在请求开始时捕获 active generation，以短 `REPEATABLE READ` 事务与受控 `search_path` 读取同一关系快照；Headless 响应带 `X-Cms-Generation`，游标绑定代次，跨代次翻页返回 409 让调用方重取。
- 配置发布在提交事务中固定配置快照与基代（选定页面、部件、来源引用、素材版本与公开内容门禁一并冻结），自动配置任务产出待人工激活的发布单，手工发布单可精确选择页面 / 部件 / 整站配置；内容发布单不会携带未选择的页面或部件工作稿。
- 排期按所选 IANA 时区的墙上时间处理，不存在的夏令时时刻被拒绝，响应仍按所选时区显示；隐式发布继承内容排期；回滚重新指向历史部署并恢复其公开投影，不改写历史修订。
- 紧急撤下、身份删除 / 归档 / 下线、素材撤权与授权过期是运行时可见性门禁，历史部署不能撤销；受影响时绕过聚合静态页与 Redis 页面缓存。站点快照的 settings 走公开字段白名单，Webhook / CDN 凭据、推送令牌与验证码密钥不复制进历史关系表。

#### 编辑工作台韧性

- 编辑页有未保存改动时，导航前与页面卸载前落一份恢复副本，重进页面可继续编辑，不再只靠 30 秒自动保存。
- 素材上传改为带站点与目录归属的队列：失败项可逐项重试，重试仍写回原来的站点 / 目录。
- 修订对比改为按字符的最小差异高亮（`ins` / `del` 保留共同上下文），富文本先剥离标签再比对，Unicode 字符不再被按码元拆开。

#### 首页图表下钻

- 登录趋势按数据点下钻到当天成功 / 失败的登录日志，操作分布按模块下钻到对应操作日志；地址拼装收敛为纯函数（`dashboard-drilldown`）并有单测，目标页一次性消费深链并在同一次导航内切回列表页签，不叠加停留状态。
- 饼图下钻带上今天的自然日区间，扇区数量与点开后的列表一致（时间范围在操作日志页可见，不是只藏在请求里）；聚合出的「其他」不是单个模块，不提供下钻。
- 移除活跃度图：口径是登录去重人数、没有对应明细页，连同契约、服务端聚合查询与 Demo Mock 的 `userActivity` 一并删除，首页少一次多余的分组查询。

### Changed

- 登录失败不再锁定账号：失败按「账号 × 来源 IP」计数（`identitySecurity.loginChallenge`，默认单来源 30 次 / 多来源 5 个 IP、窗口 30 分钟），达阈值只要求验证码（响应与 MFA 挑战同构，前端展示验证码后重新提交），真正的用户凭正确密码 + 验证码始终能登录——旧实现下任何知道用户名的人都能用失败请求把真正的用户关在门外。用户列表改为显示「需验证码」并可一键清除（`POST /api/users/{id}/unlock`），会员登录同步改造；企业 LDAP 无验证码环节，只节流失败的那个来源。登录验证码答案改存 Redis（5 分钟 TTL、`GETDEL` 一次性消费），取码与验码不再要求同一节点。
- 内容列表操作列重排：预览置为首个行内按钮，下线与查看审批收进「更多」菜单；归档与回收站 Tab 使用各自的动作集合与列宽。
- 发布中心：范围列加宽并单行省略；发布详情抽屉与发布单详情改为左对齐，逐路径明细与产物条目铺满宽度，64 位部署摘要改为等宽字体并可换行，不再溢出抽屉。
- 详情面板统一为单列键值对：审批 / 修订查看器与发布单详情不再使用 `Descriptions row`（值为 20px 粗体大字），段标题统一为 6 级标题，标签右对齐、值左对齐。
- 表格列宽与单行省略收敛：站点名称 / 标识、内容标题 / 栏目列按内容宽加宽；模型自定义字段列改为按列头文案估算宽度（`[200, 360]`，列头是「模型名 · 字段名」）；超长文本一律单行省略 + 悬停查看全名。

### Fixed

- 弹层内纵向堆叠的居中错位：`Space vertical` 未声明对齐时按 Semi 默认 `align="center"` 排布，收缩到内容宽的子项（移动端筛选复选组、更多操作菜单、运行时诊断卡片、平铺导出按钮）会被整体居中并缩窄，已补齐 `align="start"`（卡片另补 `width: 100%`）。
- 首页操作分布下钻缺少时间范围：饼图口径是「今日操作分布」，深链原先只带模块，点开看到的是该模块全部历史、与扇区数量对不上；两端解析收敛到 `parseDateTimeParam`，登录日志页的局部实现与 dayjs 依赖一并去掉。
- 内容列表标题列 / 栏目列、站点列表名称与标识列在长文案下折行撑高行高，现按内容加宽并单行省略。
- 发布中心「失败」Tab 删除空态横幅，队列 / 历史 / 失败三处共用同一段列表渲染。

### Docs

- 新增《发布单与公开代次》（`docs/cms/publication-generations.md`）：构建与激活的完整顺序、公开读取的代次语义、交付与运行边界（关系复制成本、备份与角色权限要求）。

### 升级注意

- 本版含三个数据库迁移（`0010_round_titania`、`0011_fuzzy_iceman`、`0012_smiling_james_howlett`）：新增修订与工作稿（`cms_content_revisions` / `cms_content_working_copies` / `cms_content_review_revisions` / `cms_content_revision_approvals` / `cms_content_preview_grants`）、发布治理（`cms_releases` / `cms_deployments` / `cms_site_generations` / `cms_release_activations` / `cms_content_suppressions` / `cms_distribution_sync_states`）、模型与素材版本（`cms_model_versions` / `cms_model_unique_values` / `cms_asset_versions` / `cms_asset_rights`）与审稿批注（`cms_editorial_notes`），并给 `cms_contents` / `cms_models` / `cms_model_fields` 补字段。升级前请先执行 `npm run db:migrate`。
- 已有内容在首次写入（编辑保存 / 投稿 / 采集 / 分发同步 / 站点迁移）时按需补齐工作稿与初始修订，无需手工回填；`npm run db:seed` 会为演示数据补齐工作稿与 `checkpoint` 修订，可重复执行。
- 部署数据库角色需要**创建派生 schema 的权限**（发布构建会创建 `cms_generation_*`）；备份必须包含 `cms_generation_*`，共享文件存储必须包含各 generation 目录，历史代次用于回滚、不应直接删除。
- 静态产物的目录由按站点切换为按代次（`CMS_STATIC_ROOT/{siteCode}/generation-{id}/`）：应用内的静态读取已按激活代次解析，但若有 Nginx / CDN 直接映射了旧的站点静态目录，需要同步调整；旧产物在确认新代次激活生效后再清理。
- 「上线」的判定改为发布单激活：调用 `POST /api/cms/content/{id}/publish` 或站点配置发布后，请到发布中心确认发布单进入 `active`，不要再以「任务已提交」当作生效依据。
- 本版无新增权限项：发布单与预览沿用 `cms:content:*` 权限，数据范围按发布单选中的内容与栏目收敛；配置发布与整站回滚需要完整栏目范围。
