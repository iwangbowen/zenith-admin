...
- 关联分组可筛选：关键词、业务状态、时间范围与「仅需处理」在授权 SQL 的 `LIMIT` 之前执行，覆盖完整可见集合；分组通过 `filters` 声明自身支持的过滤能力，未支持的条件返回 400，不再静默忽略。游标加密并绑定规范化后的筛选条件，不能跨筛选复用。
- 「仅需处理」按整个可见集合判断、不以最新一条代替：退款待审批 / 失败 / 结果未知、投诉未处理 / 处理中、风控待审核、流程确有运行中的待审批任务、通知与后台任务失败、响应码不小于 400 的操作日志；历史风控命中与正常执行中的任务不视为异常，未定义处理规则的关系只判断有无记录。
- 需处理记录提供处理入口，进入原模块的退款审批、任务处理、告警处理或流程详情：确认、幂等、审计与写入鉴权仍由原业务服务负责，关联模块不另建一套写操作，处理完成使关联缓存失效。
- 时间线支持事件类型与时间范围过滤，事件可点击来源对象进入精确详情并保留返回上下文；补齐异步任务终态（成功 / 失败 / 取消）与通知投递结果的结构化事件，事件与实际状态在同一事务提交，内容保持安全摘要。
- 摘要查询按每批最多 8 组 `EXISTS` 合并为单条 SQL（复用权限条件），单连接内串行执行并以 savepoint 隔离单组失败，总预算不足时只把尚未完成的分组标为 `unavailable`；多态关系继续走有界授权扫描。
- 会话内返回栈：从关联进入处理或来源详情再返回时重建精确详情参数，并恢复关联页签、展开分组、详情滚动位置与通用列表的筛选草稿 / 提交条件 / 分页；返回栈按登录用户、租户视角与模拟登录身份隔离，普通导航 / 退出登录 / 身份切换结束这次浏览，整页刷新不持久化。
- 缓存失效改由实际事件驱动：后台统一订阅支付、流程、任务、站内信与设备事件，以 200ms 合并窗口刷新活跃关系查询（只更新进度数值的任务推送不重复触发）；无可靠推送的变更由前台活跃查询每 30 秒校准，后台窗口与隐藏页面暂停。
- 来源摘要收敛：`origin.explanation` 描述公开的关联依据，`eventType` 只包含安全事件标识，`relatedAt` 只在真实持有建立时间时填写，与业务记录时间的 `occurredAt` 不再混称。

#### 人工关联语义

- 人工关联从单一「相关对象」扩展为受控目录：对称的「相关对象」，以及有方向的「补充材料 / 材料用于」「参考依据 / 被引用于」「后续处理 / 前置事项」，反向关系按反向标签展示。
- 关联可填写说明，随关联返回建立人与真实建立时间；同一对象对的不同类型关系可共存，解除时明确类型与方向，两端在读取与写入时重新授权，建立与解除均记审计。

#### 三条完整业务链

- 会员 / 钱包流水 / VIP 续费履约：会员、支付订单与实际入账 / 续费记录双向查询，履约详情展示余额或延期结果。
- 设备 / OTA 任务 / 固件 / 设备升级结果：沿任务、固件与单设备执行记录追溯升级，新增 IoT OTA 记录详情与告警事件。
- 知识文档 / 公告 / 业务附件：文档与公告的受控附件，以及同一托管文件的可见业务引用，托管文件 UUID 与网盘节点 ID 分别授权。

#### 对象关注与业务变化提醒

- 关联视图新增图标形式的关注入口，首批支持支付订单、退款、流程实例、设备、设备告警与异步任务：没有可靠业务事件或精确详情路径的对象不显示入口，关注从当前时刻开始、不补发过去事件。
- 新增 `entity_watches` 与 `entity_watch_events`（迁移 `0009_whole_puff_adder`）：领域事件与业务更新同事务写入待派发记录，异步任务成功 / 失败 / 取消与 IoT 告警触发 / 认领 / 手动或自动恢复均走该边界；没有全局事件 ID 高水位，较小 ID 的事务晚提交也不会遗漏。
- 新增通知事件「关注对象的业务变化」（`platform.entity.changed`）：默认进入站内信，可在个人通知偏好中启用邮件或关闭；点击提醒经共享详情路由目录直达事件来源详情，与前端关联下钻共用同一套安全路由。
- 派发由启动时注册的 `entity-watch-delivery` 系统任务承担（每分钟一轮，可在调度中心手动执行；单轮最长 40 秒 / 200 批次、每批 20 个关注）：租约令牌隔离并发 worker，通知入队与逐关注进度同事务提交，失败按退避重试，进程退出后租约到期继续处理。
- 授权即时生效：入队前与实际渠道发送前都重新检查用户启用状态、有效角色与菜单权限、租户活性与归属、模块许可、数据范围以及关注对象和事件来源的可见性；取消关注、删除对象、禁用用户或撤销权限后，尚未开始投递的提醒被抑制并在投递日志记 `source_unavailable`。
- 通知按事件与收件人去重（同时关注订单与退款时同一次退款只产生一条），通知自身的 queued / dispatched / failed 元事件不进入关注派发，避免递归；关注邮件沿用事件标识逐条发送，不进入会丢失对象授权边界的通用摘要 HTML。
- 前端新增关注按钮与三处业务链详情（会员履约、IoT OTA 记录、受控业务附件）；Demo 模式支持关注状态，以及支付 / 退款成功、订单关闭、流程终态、任务终态与设备告警变化的站内信演示。

### Changed

- 数组标签列全量收敛到 `overflowTagColumn`：用户岗位、成员标签、AI 评测平均分与得分、事件质量问题、质量维度明细、报表告警与订阅通道、错误告警渠道、物模型服务与事件参数、调度节点角色、作业类型 / 状态等不再手写 `Space` 平铺，溢出标签统一收进 `+N` 气泡。
- 标签列支持逐标签业务配色：`OverflowTagItem` 新增可选 `color`（缺省回退列级 `tagColor`），成员标签、告警渠道、评分好坏色等由数据决定颜色的列迁移后不丢色彩区分。
- 标签列新增固定计数收起：标签集合固定、文案长度相近的列（开放应用 Scope / 授权类型 / Webhook 订阅事件 / 脱敏豁免权限）传 `maxTagCount` 走轻量计数收纳，与默认的按列宽自适应共用同一列工厂，页面不再直接写 `TagGroup maxTagCount + showPopover`。
- 复合单元格的标签收纳：IoT 设备「属性快照」不再只展示前两个属性（隐藏项经 `+N` 悬浮查看），网盘「名称」列不再丢弃第 3 枚以后的标签，CMS 内容「标题」列最多九枚标记（顶 / 荐 / 热 / 投稿 / 附件数等）按列宽收纳，标记不再把标题挤出可视区。
- 知识库标签颜色与流程分类颜色改用偏好设置的主题色板（`THEME_COLOR_PRESETS` 与 SwatchColorPicker）：与系统标签同一套预设加自定义取色，已存档的 hex 值继续兼容。
- 流程监控「引擎诊断」的事件与调度改为纵向分节、每个表格占满宽度，并按既有小节标题模式补齐标题与数量。
- 前端分包调整：引用型元数据（实体引用 / 目录 / 详情路由 / 时间线事件标签 / 对象关注目录 / 人工关联目录）独立为 `entity-primitives`，关联运行时只保留延迟加载的组件与契约（触发组件与共享引用不再被拽进该分包）；时间线事件标签从 `domain-events` 拆成独立的纯数据模块（`entity-timeline-events`），避免引用元数据又被领域常量拖回共享包。审批端合并若干零碎 vendor 分组，首屏与延迟加载边界不变。
- 一批表格列宽与单行省略收敛：告警事件「实际值」列加宽到 130（bps 类指标的「1023.9 MB/s」在原 110 定宽下会折行）、会员等级「权益」列收纳为单行、充值记录「订单号 / 会员」列加宽、会员流水「会员」列加宽、页面搭建「区块构成」列收纳为单行、个性化菜单「一级按钮数」列加宽、群发消息「群发对象」列加宽、带参二维码「场景值」列与生成弹窗标签不再折行、任务中心类型表拆分类型列并消除模块列折行、Docker 容器列表「镜像 / 状态」列标签限宽、告警规则左侧固定列表的选择框列随规则名固定。

### Fixed

- 修复 Demo 构建下会员端入口白屏：延迟加载的关联运行时在模块顶层读取的枚举来源（时间线事件类型、人工关联类型）落在与 `entity-discovery` 互相 import 的共享 chunk 里，形成跨 chunk 环后取到 `undefined`，模块求值即抛 `Cannot convert undefined or null to object`，会员端整页无法挂载（构建仍 exit 0，只有启动冒烟能拦住）。
- 关联导航返回后不再丢失上下文：来源详情的页签、筛选与展开位置，以及通用列表的筛选草稿 / 提交条件 / 分页都会恢复；已被消费的深链参数只重建一次，不重复跳转、不叠加弹层。
- IoT 告警与设备的详情动作补齐授权并在各列表 / 详情入口同步缓存，Demo Mock、表单工具与契约快照同步（`iot-alarms`）。
- 关注派发的边界补齐：通知解析、保留策略与继续游标按新表 / 新契约对齐，`entity_watch_events` 不会被当作日志提前清理；前端通知策略 Mock 与真实契约一致。

### Docs

- 新增《对象关注与业务变化提醒》（`docs/guide/entity-watches.md`）：可靠事件与派发、权限与取消、接入新对象的清单。
- 新增《跨对象关联视图：业务处理增强》（`docs/guide/entity-relations-next-phase.md`）：本轮六个阶段与统一验收标准。
- `docs/guide/entity-relations.md` 补充分组筛选与处理入口、人工关联目录、关注与共享详情路由目录，`entity-relations-qa.md` 与 `entity-relations-roadmap.md` 同步验收记录与实现边界。

### 升级注意

- 本版含一个数据库迁移（`0009_whole_puff_adder`）：新增 `entity_watches`（用户关注，按用户 / 租户 / 对象唯一）与 `entity_watch_events`（待派发队列，随 `domain_events` 级联删除）。升级前请先执行 `npm run db:migrate`。
- `entity_watch_events` 是可靠待派发队列，派发成功后即删除并随 `domain_events` 级联回收，不要按日志保留时间预先清理。
- 本版无 seed 变更：关注入口复用现有关联视图权限，通知事件「关注对象的业务变化」默认站内信，需要邮件可在个人通知偏好中自行开启（继续遵守免打扰与摘要窗口）。
- 自定义接入需同步：新增 `GET / POST / DELETE /api/platform/entities/{type}/{key}/watch`；人工关联从单一 `platform.related` 扩展为四个受控类型，写入需携带 `relationType` 与 `direction`；分组查询支持 `filters`（关键词 / 状态 / 时间范围 / 仅需处理），未声明能力的条件会被拒绝而不是忽略。
...
- 关系来源语义：关系项新增 `origin`（来源 `kind` + 事件标识），说明每条关系为什么存在；事件标识只承载服务端给出的安全标识，不携带领域事件 payload 等敏感详情。
- 定性关联摘要：关联分组摘要只返回 `has-data` / `empty` / `attention` / `unavailable` 四态；Provider 优先实现 `summarize()` / `exists()`，未实现的按同权限 `list(limit=1)` 兜底。摘要查询改为有界并发（每批 3 个）并复用统一关系预算，详情页不再逐组串行等待。
- 关联导航返回栈：从关联对象进入精确详情时保留来源 URL、页签、筛选与滚动位置，目标详情可返回原对象上下文而不叠加 SideSheet；来源 state 经运行时校验并限制深度，外部或损坏的 state 不会改变授权边界。
- 关系可观测性：新增 `zenith_entity_relation_summary_states_total`，按来源类型 / 关系 key / 摘要状态记录降级、超时与空结果。

#### 用户与权限

- 创建与编辑用户组时可直接配置角色并同步角色继承关系，用户组列表补充角色摘要。
- 创建静态用户组时可直接选择成员；动态用户组继续由规则计算。
- 「最终有效权限」按菜单展示权限来源（角色 / 用户组 / 直接授权），便于解释权限继承。

### Changed

- 用户管理「更多」菜单的「强制下线」改为常驻：用户离线时仍展示入口，置灰并提示「该用户当前不在线」，不再整项隐藏。
- 用户角色列与租户套餐功能列改用溢出收纳（OverflowList + Popover），被收纳的角色 / 功能名经悬浮查看，内容不再撑破表格。
- 定时任务「执行概览」拆分任务与处理器列并重排关键统计列宽，执行时间提供相对时间悬浮提示。
- 服务监控历史趋势刷新收敛到统一的刷新控制，页面级自动刷新与手动更新共用一套状态。
- 站内信「收件记录」列宽与截断口令统一：内容列加宽到 360，收件人加宽到 160，收件人与发送人改为单行省略——收件人是昵称优先的展示名，长值出 tooltip 而不是换行撑高表格行；发送人空值仍显示「系统」。
- 通知策略「事件策略」的事件列加宽到 360 并改为单行省略：最长事件名（如「错误监控告警（前端错误 / 服务端异常）」）原本贴近列宽临界会折行，文本超出省略出 tooltip，「必达」锁图标固定在右侧不参与压缩。
- 通知策略「投递日志」列宽与截断口令统一：渠道加宽到 120、收件人到 200、归因到 240，三列均改为单行省略出 tooltip——原宽度下渠道标签（最长「聊天卡片」）与归因文案贴近临界就会折成两行。
- 「关联信息」业务时间线的「刷新」改为图标按钮（带 tooltip 与无障碍名称）：与「关联记录」页签的刷新同形，不再和说明文案抢同一行宽度。
- 流程详情 Tab 顺序调整：「流转记录」移到「关联信息」之前，审批轨迹不再排在低频的关联视图后面。
- 流程详情「流转记录」的审批人列加宽到 160 并改为单行省略：审批人是昵称优先的展示名，长值出 tooltip 而不是换行撑高表格行（实例监控 / 我的申请 / 审批详情 / 触发器执行等复用入口同时生效）。
- 「我的申请」操作列新增行内动作：审批中且允许撤回的申请可直接行内确认撤回，行内「更多」菜单另提供单条催办（带可选留言），两者都不必先打开详情；详情面板内的撤回与催办入口保留。

### Fixed

- 流程详情面板不再因后台回源而卸载重挂：占位符 / 骨架只在首次加载时出现，在「沟通」发完评论或审批后页签不再被重置回「表单」（我的申请、抄送我的 / 我已办、待我审批、流程监控的详情共用同一修复）。
- 菜单权限弹窗按页签区分底部操作区，「直接授权」页签不再因切到「最终有效权限」而失去保存入口。
- 权限来源与用户组来源的标题 / 标签布局对齐，消除缩进与间距不一致。
- IoT 告警与设备补齐关联详情深链。
...
- 支付订单、退款、投诉、风控、回调、日志等详情支持关联下钻：在同一详情内打开来源业务对象，不堆叠抽屉；退款关系支持全详情 Sheet 打开。
- 支付明细深链可消费：台账与日志的详情链接按原子方式消费并定位到关联详情。
- 新增支付风险与分润详情下钻入口，以及回调审计与通知下钻目标。

#### 定性关联摘要

- 新增定性关联摘要：关联对象可展示定性摘要状态，刷新时保留摘要内容、布局保持稳定，摘要指示器行为已记录。

### Changed

- 行为分析各页签表格整理：数值列去掉列内进度条（只留数字，列宽同步收窄）；页面 / 功能 / 元素（含挫败点击榜）拆成「名称 + 路径 / 标识」两列。
- 异常详情 Sheet 扁平化：诊断文本块收敛到单一来源，事件表格自适应列宽；链路追踪页只重写自身的查询参数。
- 操作日志关联列固定在右侧冻结区内；追踪查询支持从输入框清除图标重置。

### Fixed

- 不可关闭页签（首页）保留关闭按钮占位，与可关闭页签保持同宽。
- 支付详情路由快照测试同步更新。
...
- 加载动画由 8 种增至 16 种：新增环绕圆点（容器自转带动两极圆点绕圈）、九宫格（9 格按行 + 列错帧，亮点沿对角线扫过）、绕行方块（沿正方形轨迹逐角平移）、渐显省略号（原地依次亮起，唯一不位移的形态）、游动圆点（沿轨道左→右的单点，在两端淡入淡出）、落地弹球（球 + 地面影子，下落加速、触地压扁）、钟摆、流动虚线（轨道 80px / 可视区 40px / 虚线周期 10px，循环接缝不跳格）。
- 全部形态只动 `transform` / `opacity` 并纳入合成层清单，`prefers-reduced-motion` 下统一退化为呼吸动画；偏好搜索关键词与测试断言改由选项表派生，并新增两条全量守卫测试（每种已声明的动画都有中文名、都渲染出可见标记）。

#### 双列菜单

- 新增「双列首列形式」偏好：默认仅图标（与单列收起侧边栏同宽，名称靠悬浮提示），可选图标 + 文字；仅在双列布局下适用。

#### 用户管理

- 行操作新增「登录与操作记录」：抽屉内两个 Tab 复用个人中心的登录记录 / 操作记录面板，服务端按 `userId` 精确筛选（`username` 是模糊关键字，定位不到确定的一个人）；两个 Tab 分别由 `system:log:login` 与 `system:log:operation` 门控，未激活的 Tab 不发请求。

#### 收件记录

- 标题列改为主弹性列（窗口越宽越宽，内容固定为摘要），新增详情弹窗，并接上收件人与来源（手动 / 测试 / 系统 / 开放 API）筛选。
...
- 聊天模块更名为「会话中心」：菜单标题、许可目录、导出模块名、系统任务模块名、频道测试发送提示，以及文档与 README 同步；聊天页左栏去掉「消息」标题（未读总数改挂在「会话」页签上）。
- 「我的消息」更名为「站内信」：顶栏下拉入口、用户菜单、铃铛弹层与空态、到站提示、收件箱文案、菜单标题与文档同步（`/inbox` 路由与 `in_app_messages` 契约不变）。
- 「即时通讯」术语全站统一为「会话中心」（接口目录域标签、文档站导航、领域文档）；频道契约的审计模块名由「消息中心」改为「消息渠道」。
- WebSocket 监控页清理 SonarLint 告警（提取嵌套三元表达式、排序补比较函数）。

### Fixed

- 手工发送的站内信落库时来源被记为系统：`insert` 行现在显式写 `source = manual`（此前漏写、落到列默认值 `system`，人工群发被记成系统来源），`adminList` 增加 `source` 筛选。
- 数据库备份产物可被未登录读取：登记 `managed_files` 时未写 `visibility`，落库取列默认值 `public`，整库 dump 经通用文件接口即可下载；现显式登记 `restricted` + `live`，新增专用鉴权下载路由，删除备份记录改为将归档置 `orphan`。
- 打开偏好设置时内容被拽到「加载动画」那一屏（定位改用容器自身 `scrollLeft`，不再 `scrollIntoView`，也不再联动滚动抽屉正文）。
- 搜索中心「最近搜索」标题与标签贴在一起。

### Docs

- `docs/frontend/preferences.md` 增加「新增加载动画」一节：扩展点只有选项表、指示器标记、样式三处，其余（zod 枚举、选择行、搜索关键词、后台策略编辑器）都从选项表派生。
- `docs/backend/audit-log-changes.md` 与 `docs/iam/index.md` 补充 `userId` 精确筛选与用户管理行操作的说明。

### 升级注意

- 本版无数据库迁移。
- 已有环境的菜单标题不会被 `seed` 回写（`onConflictDoNothing`）：「会话中心」（5000）与「站内信」（13）需在「系统管理 → 菜单」手工改，或重新 seed。
- 频道契约的审计模块名只影响新产生的审计记录，已写入的历史记录仍为旧值；手工发送的站内信来源修复同样不回溯，已产生的记录仍为 `system`。
...
- 系统监控新增「WebSocket 连接」页：在线连接、最近断开、消息流（最近 200 条脱敏元数据）与节点 / Topic 聚合四个视图，5 秒节拍刷新并可暂停。
- 连接明细补充来源：握手时采集客户端 IP 与 User-Agent，解析出浏览器、系统与端形态；连接画像记录最后一条消息的类型、方向与时刻。详情面板可复制连接 ID、Token 与 IP，并列出同会话多标签页、同用户其他会话与该连接的最近消息。
- 断开记录带原因分布、持续时长与收发计数，可按原因 / 节点筛选。
- 集群合并视图：各 api 节点按 30 秒节拍发布本地快照，失联节点超过 90 秒自动移出；跨节点计数求和、用户去重、明细各截断 50 / 50 / 200 条后重算聚合，不再只反映命中的那个进程。
- 关系拓扑视图：扇出总线 → 网关节点（负载与实时速率） → 用户（端形态 / 多端 / 疑似重连） → Topic（下发采样边），支持点选高亮与边下钻联动，并附客户端类型分布与疑似重连表。
- 监控的计算逻辑（健康度、Topic 方向聚合、客户端统计、拓扑构建与重连推断）收敛为 shared 纯函数并补齐单测。

#### 日志查看器

- 路径选择改为「项目日志 / 目录浏览 / 常用路径」三类来源，项目日志与外部路径互斥。
- 目录浏览支持面包屑跳转任意上级目录、路径搜索与日志文件过滤；常用路径按主机类型区分，本地补 Windows 快捷项，浏览时支持盘符切换。

### Changed

- WebSocket 连接页 5 张表格统一改用 ConfigurableTable：每表一个弹性主列、支持列设置、统一刷新入口，窄屏横向滚动不再破版。
- 日志查看器放开目录白名单，任意绝对路径可读；来源工具栏压缩（主机选择器尺寸对齐、去掉重复标题行），路径选择弹窗样式统一。
- 关系拓扑的客户端类型分布由卡片列表改为统计条与表格，去掉浏览器 / 系统 Top 列表。

### Fixed

- 收敛 WebSocket 连接监控的可见范围：按平台 / 租户与管理范围过滤连接、断开与消息明细（非平台管理员看不到平台超管的连接），派生统计按裁剪后的明细重算。
- 修复 WebSocket 连接详情面板不跟随刷新——打开后收发计数、持续时间与最近活动一直停在选中那一刻。
- 修复关系拓扑的稳定性问题：排布确定性化、刷新后保留节点位置、视口不再跳动，下钻保持原位。
- 心跳（ping / pong）不再计入业务拓扑的投递边。
- 修复弹窗关闭后详情列表的滚动位置与消息焦点丢失（收件箱），并把视口恢复逻辑集中到 AppModal。
- 修复日志查看器已选项目无法清空、切换日志来源后选择状态未重置。
- 修复下拉菜单未读徽标配色与行内描边不一致。
...
- 支付对账中心补齐账单、差异案件、核对历史、调整审批和执行任务的联动刷新与深链导航。
- 核对历史和执行任务支持行内展开查看错误详情，差异案件处理历史使用 Timeline 展示。

#### 全局搜索

- 统一搜索弹窗与搜索中心的类型、URL 状态、最近搜索、错误反馈和无障碍交互。
- 顶部搜索入口统一显示为「全局搜索」，菜单入口统一显示为「搜索中心」。

### Changed

- 支付对账页面的账单 SideSheet、差异案件详情和展开详情改为分区平铺布局，长文本统一省略。
- 支付对账表格的操作列、表单标签和错误信息展示进行布局收敛，避免内容挤压与换行。
- 个人中心多因素认证列表取消卡片容器样式。

### Fixed

- 修复差异案件在核对证据变化后仍保持忽略状态的问题。
- 修复全局搜索跳转时筛选条件丢失、顶部搜索入口焦点残留等问题。
...
- 用户表新增出生日期字段（需执行数据库迁移），用户管理弹窗与个人中心资料编辑均可维护。
- 偏好新增顶栏时钟与定时深色。
- 界面与体验模块新增「偏好设置入口」开关，关闭后隐藏头像下拉菜单中的偏好设置（含锁屏密码设置）。

#### 终端识别与审计

- 登录与模拟登录支持自报精确系统信息（Client Hints 区分 Win11），会话迁移沿用终端信息；操作日志与会话指纹读取 Client Hints。
- 操作日志 OS 列改用 JWT 签名断言（回退自报头，审计面仅信服务端 UA + CH + 令牌断言）；身份文档同步补充操作系统检测口径说明。

#### 文件与设置

- 文件列表与文件配置浏览支持点击文件名预览（仅可预览类型可点，与操作列预览同口径）。
- 设置模块详情头部新增覆盖数标签。

#### 体验增强

- 统一头像选择弹窗，裁剪改受控旋转缩放。
- 欢迎区显示当前日期星期，登录信息改为本次登录。
- 提醒音效新增 4 种音色。
- 菜单与树形、工作流、聊天、选人、运维与 AI 本地搜索支持拼音。

### Changed

- 用户管理新增弹窗改为密码在前、手机号码在后；性别与出生日期移至角色之后，列表性别列移至角色列之后。
- 个人中心基本信息去卡片化，手机号与邮箱的展示和输入位置互换。
- 设置模块列表标签精简防溢出（专用页面改为常驻图标加悬停提示），标题行展示版本号 `vN`。
- 顶栏用户下拉菜单执行动作前先收起；顶栏右侧贴边，头像箭头与标签栏开关对齐。
- 自报浏览器 / OS 统一按列宽截断兜底，缺项时回退 UA 解析，避免浏览器变 Unknown。

### Fixed

- 补全签名审计用例的 request-helpers mock。
...
- 新增系统级偏好策略：管理员在「系统设置 → 界面与体验 → 偏好策略」设置每项偏好的系统默认值与是否允许用户修改；禁止修改的项目在个人偏好中隐藏并强制使用系统值，锁定期间保留用户原选择、解锁后恢复。
- 个人偏好改为只保存明确覆盖：新增单项「跟随系统」与「恢复系统默认」，导入导出统一为 `{overrides}` 文档；策略保存后经实时通道推送，多页面自动同步。
- 偏好依赖显式声明：子项按父项最终生效值显隐（如面包屑关闭时相关子项隐藏），后台始终可配全部项目并展示适用条件。

#### 个人中心

- 登录记录 Tab 新增事件类型、状态、时间范围筛选；操作记录 Tab 新增模块、操作描述、请求方法、请求路径、IP、执行结果、操作来源、时间范围筛选，我的操作记录接口同步扩展相应查询参数。

#### 偏好设置体验

- 加载动画由 4 种增至 8 种：新增菊花转、双弹跳、水波纹、进度条；选择行改为单行横滑（无滚动条、选项尺寸不变），两侧箭头导航，当前选中自动滚入可见区。
- 偏好底部操作改分裂按钮：主按钮复制偏好，下拉承载导入偏好与新增的导出配置文件（`zenith-preferences.json`，可回贴导入）。
- 知识标签颜色支持自定义取色与清空：保留 8 个固定预设，复用与系统标签同款的色板选择器。
- 设置模块列表新增版本号标签，未修改过的模块不显示。

### Changed

- 系统标签颜色选择器复用偏好同款色板（主题预设加自定义取色加无颜色），新增标签弹窗加宽；决策表 Key 列改可复制列并加宽至 200，超长省略加悬停查看加一键复制。
- CMS 站点主题配置的原生取色框改用 Semi ColorPicker。
- 缓存管理剩余 TTL 改中文习惯展示（2小时14分）。
- 偏好设置搜索框占满整行；通讯录同步源字段映射改单列，避免输入溢出。

### Fixed

- 修复新增公告「定时发布时间」标签折行的问题。
...
- 在线用户与用户管理新增「最近活跃」列：在线会话直接展示，活跃时间由请求心跳按分钟精度回写；用户管理按用户聚合最新活跃时间，离线显示占位符。
- 普通用户与 CMS 编辑种子角色补齐个人中心、公告中心与我的消息菜单。

#### 消息与首页

- 我的消息详情弹窗支持跳转关联业务页面：带深链的消息在列表中标识，详情弹窗增加「前往处理」按钮。
- 首页系统概览四个统计（用户总数、当前在线、今日登录、今日操作）支持点击跳转对应管理页面。

### Changed

- 同域子路径派生项目新增 `VITE_DEPLOYMENT_ID` 部署标识，隔离浏览器认证状态、本地偏好、草稿、埋点缓存与 PWA Cache Storage；旧的无 namespace 浏览器 key 不再读取。
- 会话「设备 / 浏览器 / 系统」列加宽并单行省略显示，设备不再使用特殊标签样式。
- 收起态侧边栏宽度由 48px 收窄至 40px；顶栏高度由 48px 降至 40px。
- 个人中心通知设置 Tab 前移至账号安全之后。

### Fixed

- 修复普通用户打开已发布文档时评论 @选人下拉触发无权限提示的问题（改为按权限加载）。
- 用户下拉菜单的移动审批入口按审批权限隐藏，无审批权限的用户不再看到入口。
- 修复角色与用户管理菜单分配弹窗的双滚动条问题。
- 侧栏换边按钮改用双向箭头图标，消除与折叠的歧义。
- 修复新建 API Token 弹窗名称标签换行问题。
...
- 工作流筛选、关联审批单和填报模板绑定新增独立流程选项查询，支持业务系统主导（`external`）流程；直接发起流程仍按业务规则限制。
- 所有密码与密钥输入统一使用带显隐按钮的密码输入组件，覆盖管理后台、会员端、审批端和系统配置；短链访问密码页同步支持显隐。

#### 个性化设置

- 新增 Tab 类型与紧凑 Tab 栏偏好，统一应用于页面 Tab 和内容 Tab。
- 新增弹窗点击遮罩关闭偏好。

### Changed

- 个人中心资料编辑与用户管理统一邮箱选填、清空和格式校验规则。
- 工作流与报表页面的流程选项按“可发起流程”和“记录 / 关联流程”场景分离，避免权限和表单类型误过滤。
- 保留策略页面的描述与模块 / 标题列支持更完整的展示；SQL 监控时间列统一使用共享时间组件。

### Fixed

- 修复流程自动化新增规则时“所属流程”下拉为空的问题。
- 修复审批单打印模板选择行按钮显示不完整的问题。
- 修复表格斑马纹下固定列透明、侧边栏滚动条与边框对齐，以及个人签名编辑间距问题。
- 优化恢复类通知的文案区分，并恢复 SQL 监控时间列的正确渲染。
