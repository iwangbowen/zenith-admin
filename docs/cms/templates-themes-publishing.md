...
- 内置主题显式选择模板时，当前站点 DSL → 显式继承父级 DSL → 主题级全局 DSL → 内置变体/默认模板；运行时、健康检查与选择器共用同一 resolver。
...
父级主题/模板变更会先分析实际继承且未被子站同名模板覆盖的站点；操作人必须具备全部受影响站点/栏目的 ACL。每个站点独立递增 revision 并提交 fenced 重建。站群整组发布使用 `cms:publish:group`，同样为每个目标站点提交现有 `cms-publish-build`，不新建发布队列。详见[站群层级、继承与内容分发](./site-groups-and-distribution)。

## 权限

- 模板：`cms:template:view|manage|activate`
- 主题：`cms:theme:view|import|activate|export`
- 发布：`cms:publish:view|build|manage|group`
