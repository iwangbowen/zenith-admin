...
| 进程管理 | 进程列表实时推送、资源占用、详情、网络连接、结束进程、优先级调整、导出中心 Excel / CSV 导出 |
...
实时列表通过 `GET /api/processes/stream` 以 SSE 推送，首帧返回完整列表，之后每 3 秒刷新一次，并每 30 秒发送心跳。页面支持关键字与状态筛选、详情弹窗、结束进程、优先级调整以及通过导出中心导出 Excel / CSV。
...
进程列表导出统一通过导出中心创建任务，筛选条件沿用进程列表当前提交查询参数。
