...
- 关键词即时高亮（防抖）与上一个 / 下一个匹配导航（Enter / Shift+Enter）；「全文」模式通过服务端 `keyword` 参数过滤整个文件后返回匹配行；
- `ERROR` / `WARN` / `INFO` / `DEBUG` 级别着色与筛选，无级别标记的堆栈续行继承上一行级别；
- 实时追踪智能跟随：仅当已滚动到底部时自动跟随新日志，向上翻阅不被打断，可一键「回到底部」；断线自动重连（连续 3 次失败后停止）；
- 行号、自动换行、加载行数等偏好本地持久化。
