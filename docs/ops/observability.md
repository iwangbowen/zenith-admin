...
页面顶部的更新方式与刷新按钮作用于当前标签页：实时指标支持 SSE（约 10 秒一次）、定时刷新和暂停；历史趋势默认每分钟重新查询，也可暂停自动更新，二者分别记忆偏好。进入历史趋势时断开实时订阅并停止实时轮询，返回实时指标时恢复其更新方式。SSE 模式下手动刷新通过重连获取完整首帧，避免 HTTP 快照与差量推送交错覆盖。
