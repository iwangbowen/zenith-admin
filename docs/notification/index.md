...
通知中心覆盖邮件、短信、站内信与公告四类能力。后端统一挂载在 `/api` 前缀下，业务表集中在 `packages/server/src/db/schema/messaging.ts`（通知模块）与 `announcements.ts`（公告模块）；前端页面集中在「系统设置」下的通知管理、公告管理，以及顶部铃铛入口对应的个人收件箱。
