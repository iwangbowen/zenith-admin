...
const create = defineOpenAPIRoute({
  route: createRoute({
    ...
    summary: '创建 AI 服务商配置',
    security: [{ BearerAuth: [] }],
    middleware: [authMiddleware, guard({ permission: 'ai:provider:create', module: 'AI服务商', action: '新增' })] as const,
...
const update = defineOpenAPIRoute({
  route: createRoute({
    ...
    summary: '更新 AI 服务商配置',
    security: [{ BearerAuth: [] }],
    middleware: [authMiddleware, guard({ permission: 'ai:provider:edit', module: 'AI服务商', action: '编辑' })] as const,
...
const remove = defineOpenAPIRoute({
  route: createRoute({
    ...
    summary: '删除 AI 服务商配置',
    security: [{ BearerAuth: [] }],
    middleware: [authMiddleware, guard({ permission: 'ai:provider:delete', module: 'AI服务商', action: '删除' })] as const,
    request: { params: IdParam },
...
const setDefault = defineOpenAPIRoute({
  route: createRoute({
    ...
    summary: '设为默认 AI 服务商',
    security: [{ BearerAuth: [] }],
    middleware: [authMiddleware, guard({ permission: 'ai:provider:edit', module: 'AI服务商', action: '设为默认' })] as const,
    request: { params: IdParam },
