...
const publicGetRoute = defineOpenAPIRoute({
  ...
  handler: async (c) => {
    ...
    if (!row) {
      return c.json(errBody('配置不存在', 404), 404);
    }
    return c.json(
      okBody({ configKey: row.configKey, configValue: row.configValue, configType: row.configType }),
      200,
    );
  },
});
...
const passwordPolicyRoute = defineOpenAPIRoute({
  ...
  handler: async (c) => {
    ...
    return c.json(okBody(policy, 'success'), 200);
  },
});
...
const createConfigRoute = defineOpenAPIRoute({
  ...
  handler: async (c) => {
    ...
    const [existing] = await db
      ...
      .limit(1);
    if (existing) {
      return c.json(errBody('配置键已存在'), 400);
    }
    const [row] = await db
      ...
      .returning();
    return c.json(
      okBody({ ...row, createdAt: row.createdAt.toISOString(), updatedAt: row.updatedAt.toISOString() }, '创建成功'),
      200,
    );
  },
});
...
const updateConfigRoute = defineOpenAPIRoute({
  ...
  handler: async (c) => {
    ...
    return c.json(
      okBody({ ...row, createdAt: row.createdAt.toISOString(), updatedAt: row.updatedAt.toISOString() }, '更新成功'),
      200,
    );
  },
});
