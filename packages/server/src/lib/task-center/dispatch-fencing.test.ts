...
vi.mock('../../db', () => ({ db: { select: mocks.select, update: mocks.update, insert: mocks.insert, transaction: (fn: (tx: unknown) => unknown) => fn({ select: mocks.select, update: mocks.update, insert: mocks.insert }) } }));
