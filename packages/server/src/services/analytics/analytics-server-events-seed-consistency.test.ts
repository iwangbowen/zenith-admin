...
import { ANALYTICS_SEMANTIC_EVENT_NAMES, ANALYTICS_SERVER_PAYMENT_EVENT_NAMES, ANALYTICS_SERVER_WORKFLOW_EVENT_NAMES, ANALYTICS_SERVER_MEMBER_EVENT_NAMES, ANALYTICS_SERVER_SHORTLINK_EVENT_NAMES, ANALYTICS_EVENT_NAMES, ANALYTICS_MEMBER_POINTS_EVENT_BY_TX_TYPE, ANALYTICS_SEMANTIC_EVENT_LABELS } from '@zenith/shared/analytics';
...
describe('SEED_ANALYTICS_EVENT_META 与 ANALYTICS_SEMANTIC_EVENT_NAMES 一致性', () => {
  it('事件总数为 9（系统）+ 5（支付）+ 15（工作流）+ 10（会员）+ 1（短链）= 40', () => {
    ...
    expect(ANALYTICS_SERVER_SHORTLINK_EVENT_NAMES.length).toBe(1);
    expect(ANALYTICS_SEMANTIC_EVENT_NAMES.length).toBe(40);
  });
