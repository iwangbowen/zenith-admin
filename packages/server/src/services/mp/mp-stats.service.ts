...
export async function getMpStats(accountId: number): Promise<MpStats> {
  ...
  const [fanTotal, fanSubscribed, fanUnsubscribed, tagTotal, materialTotal, draftTotal, messageIn, messageOut, autoReplyTotal] = await Promise.all([
    db.$count(mpFans, buildWhere(and(eq(mpFans.accountId, accountId), tenantScope(mpFans)))),
    db.$count(mpFans, buildWhere(and(eq(mpFans.accountId, accountId), eq(mpFans.subscribe, 'subscribed'), tenantScope(mpFans)))),
    db.$count(mpFans, buildWhere(and(eq(mpFans.accountId, accountId), eq(mpFans.subscribe, 'unsubscribed'), tenantScope(mpFans)))),
    db.$count(mpTags, buildWhere(and(eq(mpTags.accountId, accountId), tenantScope(mpTags)))),
    db.$count(mpMaterials, buildWhere(and(eq(mpMaterials.accountId, accountId), tenantScope(mpMaterials)))),
    db.$count(mpDrafts, buildWhere(and(eq(mpDrafts.accountId, accountId), tenantScope(mpDrafts)))),
    db.$count(mpMessages, buildWhere(and(eq(mpMessages.accountId, accountId), eq(mpMessages.direction, 'in'), tenantScope(mpMessages)))),
    db.$count(mpMessages, buildWhere(and(eq(mpMessages.accountId, accountId), eq(mpMessages.direction, 'out'), tenantScope(mpMessages)))),
    db.$count(mpAutoReplies, buildWhere(and(eq(mpAutoReplies.accountId, accountId), tenantScope(mpAutoReplies)))),
  ]);
