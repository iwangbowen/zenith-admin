...
export function subjectProviders(sourceType: CanonicalEntityType): readonly RelationProvider[] {
  const spec = (suffix: string, target: EntityRelationItem['ref']['type'], permission: Exclude<RelationProvider['permissions'], 'authenticated'>, list: RelationProvider['list']): RelationProvider => ({
    sourceType, key: `${sourceType}.${suffix}`, permissions: permission,
    descriptor: { key: `${sourceType}.${suffix}`, labelKey: `relation.common.${suffix}`, targetTypes: [target], kind: 'activity', cardinality: 'many', capabilities }, list,
  });
  return [
    ...
    spec('notifications', 'notification.outbox', ['system:notify-policy:list'], async (anchor, { cursor, limit, access }) => {
      ...
      const rows = await access.db.select({ id: notificationOutbox.id, title: notificationOutbox.eventKey, createdAt: notificationOutbox.createdAt, status: notificationOutbox.status })
        .from(notificationOutbox).where(buildWhere(exactTenantCondition(notificationOutbox.tenantId, anchor.tenantId), tenantCondition(notificationOutbox, access.user),
    ...
    spec('tasks', 'tasks.async', ['system:async-task:list'], async (anchor, { cursor, limit, access }) => {
      ...
      const rows = await access.db.select({ id: asyncTasks.id, title: asyncTasks.title, createdAt: asyncTasks.createdAt, status: asyncTasks.status })
        .from(asyncTasks).where(buildWhere(exactTenantCondition(asyncTasks.tenantId, anchor.tenantId), tenantCondition(asyncTasks, access.user),
          exists(access.db.select({ id: asyncTaskSubjects.taskId }).from(asyncTaskSubjects).where(and(
            eq(asyncTaskSubjects.taskId, asyncTasks.id), eq(asyncTaskSubjects.entityType, anchor.ref.type), eq(asyncTaskSubjects.entityKey, anchor.ref.key), exactTenantCondition(asyncTaskSubjects.tenantId, anchor.tenantId)))),
