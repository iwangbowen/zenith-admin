...
export async function createCmsSite(data: CreateCmsSiteInput) {
  ...
  assertSiteThemeConfig(data.theme ?? 'default', data.settings as Record<string, unknown> | undefined);
  try {
...
export async function updateCmsSite(id: number, data: UpdateCmsSiteInput) {
  ...
  if (data.settings !== undefined) {
    ...
    assertSiteThemeConfig(theme, data.settings as Record<string, unknown>);
  }
  try {
    const row = await db.transaction(async (tx) => {
      if (data.isDefault) {
