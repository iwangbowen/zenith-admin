...
export default function ContributionsPage() {
  ...
  const listQuery = useMyContributions({ page, pageSize, status: status === 'published' ? 'published' : undefined, editorialStatus: status === 'pending' || status === 'rejected' ? status as CmsEditorialStatus : undefined });
  const deleteMutation = useDeleteContribution();

  const list = listQuery.data?.list ?? [];
  const total = listQuery.data?.total ?? 0;

  function handleDelete(id: number, version: number) {
    confirmAndDelete({
      title: '删除投稿',
      content: '确定删除该投稿吗？删除后不可恢复。',
      okText: '删除',
      run: () => deleteMutation.mutateAsync({ params: { id }, body: { expectedVersion: version } }),
      successMessage: '已删除',
    });
  }
      ...
      {listQuery.isLoading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {list.map((item) => {
            ...
            const editable = item.editorialStatus === 'draft' || item.editorialStatus === 'rejected';
            return (
              <div
                key={item.id}
                style={{
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  <Tag size="small" color={meta.color}>{meta.label}</Tag><Tag size="small" color={CMS_EDITORIAL_STATUS_COLORS[item.editorialStatus]}>{CMS_EDITORIAL_STATUS_LABELS[item.editorialStatus]}</Tag>
                ...
                {item.editorialStatus === 'rejected' && item.rejectReason ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 12, color: 'var(--m-text-secondary)' }}>
                  ...
                  {editable ? (
                    <>
                      ...
                      <Button size="small" theme="borderless" type="danger" icon={<Trash2 size={13} />} onClick={() => handleDelete(item.id, item.version)}>
                        删除
                      </Button>
                    </>
                  ) : null}
