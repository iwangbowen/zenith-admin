...
/** 部门树（角色管理范围、部门管理等场景全局共享缓存）；与不带条件的 `useDepartmentTreeSearch({})` 同一缓存 */
export function useDepartmentTree(options?: { enabled?: boolean }) {
  return useApiQuery(departmentContract.tree, { query: {} }, {
...
export function useDepartmentTreeSearch(params: DepartmentTreeParams, options?: { enabled?: boolean }) {
  return useApiQuery(departmentContract.tree, { query: params }, {
    staleTime: LOOKUP_STALE_TIME,
    enabled: options?.enabled ?? true,
  });
}
...
export function useDepartmentDetail(id: number | undefined, enabled = true) {
  return useApiQuery(departmentContract.detail, { params: { id: id ?? 0 } }, { enabled: enabled && id !== undefined });
}
