 ...
 */
export function FormStatusRadioGroup({ field = 'status', label = '状态', disabled, type, extraText }: Readonly<FormStatusRadioGroupProps>) {
  const { items } = useDictItems('common_status');
  return (
    <Form.RadioGroup field={field} label={label} disabled={disabled} type={type} extraText={extraText}>
      {items.map((item) => (
