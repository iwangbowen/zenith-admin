 ...
 * 段落：标题 + 内容。
 *
 * 详情面板统一用 6 级标题、8px 下间距分组（与异常详情抽屉、异常问题详情一致），
 * 段落之间只靠 Space 的留白拉开——不再套卡片外壳，与「zx-panel 无卡片面板」的全站语言一致。
 */
function Section({ title, children }: Readonly<{ title: string; children: ReactNode }>) {
  return (
    <div style={{ width: '100%' }}>
      <Typography.Title heading={6} style={{ marginBottom: 8 }}>{title}</Typography.Title>
      {children}
    </div>
  );
}
...
export function ContentRevisionViewer({ content, fields = content.modelFields ?? [], heading = '稿件详情' }: Readonly<{
  ...
  return (
    <Space vertical align="start" spacing={16} style={{ width: '100%' }}>
      {/* 抬头：本次查看口径（审核稿 / 工作稿 / 历史修订）做小字提纲，标题与状态标签同排 */}
      <div style={{ width: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6, fontSize: 12, lineHeight: '18px', color: 'var(--semi-color-text-2)' }}>
          <span>{heading}</span>
          <span>·</span>
          <span>{content.revisionId ? `固定修订 #${content.revisionId}` : `工作稿 v${content.version}`}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <Typography.Title heading={5} style={{ margin: 0 }}>{content.title || '未命名内容'}</Typography.Title>
          <Tag>{CMS_CONTENT_TYPE_LABELS[content.contentType]}</Tag>
          <Tag>{CMS_CONTENT_STATUS_LABELS[content.status]}</Tag>
        </div>
        {content.subTitle ? <Typography.Paragraph style={{ margin: '6px 0 0' }}>{content.subTitle}</Typography.Paragraph> : null}
        {content.summary ? <Typography.Paragraph type="secondary" style={{ margin: '4px 0 0' }}>{content.summary}</Typography.Paragraph> : null}
        {content.coverImage ? <Image src={content.coverImage} alt={content.socialImageAlt || content.title} width={240} style={{ marginTop: 12 }} /> : null}
      </div>

      {/* 字段一律「标签在左、值在右」单列排布：Semi 的 row（双行）模式会把值渲染成 16–20px 粗体，
          二十来个字段并排后会像一排海报标题，且 flex 换行导致各行列宽对不齐。 */}
      <Descriptions
        size="small"
        data={[
          ...
          { key: '来源链接', value: linkText(content.sourceUrl) },
          ...
          ...(content.externalLink ? [{ key: '跳转链接', value: linkText(content.externalLink) }] : []),
        ]}
      />

      {content.mediaData.images?.length ? (
        <Section title="图集">
          <Space wrap align="start">
            {content.mediaData.images.map((item, index) => (
              <figure key={`${item.url}-${index}`} style={{ margin: 0, maxWidth: 240 }}>
                <Image src={item.url} alt={item.caption || `图片 ${index + 1}`} width={240} />
      ...
      {content.mediaData.mediaUrl ? (
        <Section title="音视频">
          {content.mediaData.mediaType === 'audio'
            ? <audio controls src={content.mediaData.mediaUrl} style={{ width: '100%' }} />
            : <video controls src={content.mediaData.mediaUrl} poster={content.mediaData.poster} style={{ width: '100%', maxHeight: 420 }} />}
          <Typography.Text type="tertiary">{content.mediaData.duration}</Typography.Text>
        </Section>
      ) : null}

      {content.body ? (
        <Section title="正文">
          <div className="cms-revision-body" style={{ overflowWrap: 'anywhere' }} dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(content.body) }} />
        </Section>
      ) : null}
      ...
      {extendEntries.length ? (
        <Section title="模型字段">
          <Descriptions size="small" data={extendEntries.map(([name, value]) => {
