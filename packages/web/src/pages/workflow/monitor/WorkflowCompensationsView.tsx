...
export default function WorkflowCompensationsView() {
  ...
  const columns: ColumnProps<WorkflowCompensation>[] = [
    ...
    { title: '自动动作', dataIndex: 'compensationActionStatus', width: 100, render: (s: string) => (s && s !== 'none' ? <Tag color={ACTION_STATUS[s]?.color as never}>{ACTION_STATUS[s]?.text ?? s}</Tag> : <Typography.Text type="tertiary">—</Typography.Text>) },
    createdAtColumn,
