 ...
 * 提供下拉选择 + 新建表单 + 刷新 + 只读预览，替代内嵌表单设计。
 */
import { useCallback, useEffect, useState } from 'react';
import { Button, Select, Typography, Empty, Spin, Toast } from '@douyinfe/semi-ui';
...
const FORM_DESIGNER_PATH = '/workflow/forms/designer';
...
export default function FormSelectorPanel({ formId, formName, onSelect }: Readonly<FormSelectorPanelProps>) {
  ...
  // 从表单设计器新建/编辑表单返回后自动刷新
  useEffect(() => {
    const onFocus = () => void loadForms();
  ...
  const openDesigner = (id?: number) => {
    const url = id ? `${FORM_DESIGNER_PATH}?id=${id}` : FORM_DESIGNER_PATH;
    const win = globalThis.open(url, '_blank');
    if (!win) Toast.info('请允许浏览器弹窗以打开表单设计器');
  };
    ...
    <div className="fd-form-selector">
      <div className="fd-form-selector__bar">
        ...
        <div className="fd-form-selector__bar-right">
          <Button icon={<RefreshCw size={14} />} type="tertiary" theme="borderless" onClick={() => void loadForms()}>
            刷新
          </Button>
          {selected && (
            <Button icon={<ExternalLink size={14} />} type="tertiary" theme="borderless" onClick={() => openDesigner(selected.id)}>
              编辑此表单
            </Button>
          )}
          <Button icon={<Plus size={14} />} type="primary" onClick={() => openDesigner()}>
            新建表单
          </Button>
        </div>
      </div>
      ...
      <div className="fd-form-selector__preview">
        {loading ? (
          ...
          <Empty
            ...
            title="未选择表单"
            description="从上方下拉选择一个已设计的表单，或点击「新建表单」前往表单库创建"
