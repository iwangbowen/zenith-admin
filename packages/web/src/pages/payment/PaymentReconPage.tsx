...
export default function PaymentReconPage() {
  ...
  const caseColumns: ColumnProps<PaymentReconCase>[] = [
    ...
    dateTimeColumn('处理期限', 'dueAt'), createOperationColumn<PaymentReconCase>({ width: 90, actions: (item) => [{ key: 'detail', label: '调查处理', onClick: () => setCaseId(item.id) }] }),
  ];
  ...
  return <div className="page-container">
    ...
    <SideSheet visible={periodId !== undefined} title="账单版本与归档原件" width={1060} onCancel={() => { setPeriodId(undefined); setStatementId(undefined); }}>
      <Space wrap><Select aria-label="账单版本" value={selectedStatementId} optionList={(versions.data ?? []).map((item) => ({ value: item.id, label: `v${item.version} · ${PAYMENT_STATEMENT_SOURCE_LABELS[item.source]} · ${PAYMENT_STATEMENT_STATUS_LABELS[item.status]}` }))} onChange={(value) => setStatementId(Number(value))} style={{ minWidth: 330 }} />
      ...
      {statement.data ? <><Descriptions row data={[{ key: '摘要', value: statement.data.contentHash }, { key: '解析器版本', value: statement.data.parserVersion }, { key: '校验结果', value: JSON.stringify(statement.data.verification) }]} />
        <Space wrap>{statement.data.files.map((file) => <Button key={file.id} disabled={!hasPermission('payment:recon:download')} onClick={() => void request.download(urlOf(paymentReconContract.download, { params: { id: file.id } }), file.filename)}>下载 {file.filename} ({file.byteLength} B)</Button>)}</Space>
        <Typography.Title heading={6}>标准明细</Typography.Title><ConfigurableTable columns={entryColumns} columnSettingsKey="payment-recon-entries" {...listTableProps(entries, { pagination: entryPage.buildPagination })} /></> : <Empty description="尚无可用账单，查看执行任务了解获取进度" />}
    </SideSheet>

    <SideSheet visible={caseId !== undefined} title="差异调查与处理" width={860} onCancel={() => setCaseId(undefined)}>
    ...
    <EditFormModal modal={adjustmentModal} title="创建审批调整单" width={680} header={<Banner type="warning" closeIcon={null} description="调整需要真实渠道已验证账单、明确的本地归属和金额依据；人工上传、沙箱及渠道单边差异先调查，不可直接过账。" />}>
      ...
      <Form.Input field="amount" label="金额（整数分）" rules={required} placeholder="例如 100 表示 1.00 元" /><Form.Select field="direction" label="调整方向" optionList={PAYMENT_RECON_DIRECTION_OPTIONS} rules={required} style={fullWidth} /><Form.TextArea field="reason" label="调整依据" rules={required} maxCount={2000} />
    </EditFormModal>
