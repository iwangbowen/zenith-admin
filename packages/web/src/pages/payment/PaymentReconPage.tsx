...
export default function PaymentReconPage() {
  ...
  return <div className="page-container payment-recon-page">
    ...
    <Tabs activeKey={tab} onChange={setTab} keepDOM={false} collapsible="auto">
      ...
      <Tabs.TabPane tab="核对历史" itemKey="runs"><ListSearchToolbar page={runs} filters={['status', 'statementId']} /><ConfigurableTable columns={runColumns} columnSettingsKey="payment-recon-runs" {...runs.tableProps} expandedRowRender={renderRunExpanded} rowExpandable={(item) => Boolean(item?.error)} hideExpandedColumn expandRowByClick /></Tabs.TabPane>
