  ...
  return (
    <>
      <SearchToolbar
        filters={(
          <>
            <FilterSelect<number> value={draftParams.spaceId} placeholder="全部空间" width={200} items={spaceOptions} onChange={(v) => setDraftParams((p) => ({ ...p, spaceId: v }))} />
            <Checkbox checked={draftParams.activeOnly} onChange={(e) => setDraftParams((p) => ({ ...p, activeOnly: !!e.target.checked }))}>仅保留中</Checkbox>
          </>
        )}
        actions={(
          <>
  ...
  return (
    <>
      <SearchToolbar
        filters={(
          <>
            <FilterSelect<DriveQuotaRequestStatus> value={draftParams.status} placeholder="全部状态" width={130} items={STATUS_OPTIONS} onChange={(v) => setDraftParams((p) => ({ ...p, status: v }))} />
            <FilterSelect<number> value={draftParams.spaceId} placeholder="全部空间" width={200} items={spaceOptions} onChange={(v) => setDraftParams((p) => ({ ...p, spaceId: v }))} />
          </>
        )}
    ...
    <>
      <SearchToolbar
        filters={(
          <>
            <FilterSelect<number> value={draftParams.spaceId} placeholder="全部空间" width={200} items={spaceOptions} onChange={(v) => setDraftParams((p) => ({ ...p, spaceId: v }))} />
            <InputNumber min={1} placeholder="外链 ID" value={draftParams.shareId} onChange={(v) => setDraftParams((p) => ({ ...p, shareId: typeof v === 'number' ? v : undefined }))} style={{ width: 120 }} hideButtons />
            <FilterSelect<string> value={draftParams.action} placeholder="全部动作" width={130} items={LOG_ACTION_OPTIONS} onChange={(v) => setDraftParams((p) => ({ ...p, action: v }))} />
            <FilterSelect<'true' | 'false'> value={draftParams.ok} placeholder="全部结果" width={110} items={[{ value: 'true', label: '通过' }, { value: 'false', label: '拒绝' }]} onChange={(v) => setDraftParams((p) => ({ ...p, ok: v }))} />
