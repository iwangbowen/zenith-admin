 ...
 * 揭示是原地替换、没有空间移动语义，因此固定用交叉淡化，不跟随 slide 方向（关闭动画时同样关闭）。
 */
export default function RouteSuspense({ fallback, children }: Props) {
  const reveal = useRouteAnimation() === 'none' ? 'none' : 'fade';
  return (
    <Suspense
      fallback={<RouteViewTransition animation={reveal}>{fallback ?? <PageLoading inline />}</RouteViewTransition>}
    >
      <RouteViewTransition animation={reveal}>{children}</RouteViewTransition>
    </Suspense>
  );
}
