...
export const dbAdminContract = defineContract('/api/db-admin', {
  ...
  backups: op.get('/backups', { query: dbBackupListQuery, response: paginated(dbBackupSchema), summary: '数据库备份列表' }),
  createBackup: op.post('/backups', { body: createDbBackupSchema, response: dbBackupCreatedSchema, summary: '创建数据库备份' }),
  removeBackup: op.delete('/backups/{id}', { params: idParam, summary: '删除数据库备份记录' }),
}, { tags: ['DbAdmin'] });
