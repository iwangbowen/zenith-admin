...
const metadata: Record<PreferencePath, Entry> = {
  ...
  scheduledDarkMode: { label: '定时深色', group: 'appearance', options: options(C.SCHEDULED_DARK_MODES, ['关闭', '自定义时段']), description: '时段内无条件使用深色；时段外跟随颜色模式。' },
  scheduledDarkStart: { label: '深色开始时间', group: 'appearance', applicableWhen: { field: 'scheduledDarkMode', equals: 'custom' }, description: 'HH:mm 24 小时制，如 18:00。' },
  scheduledDarkEnd: { label: '深色结束时间', group: 'appearance', applicableWhen: { field: 'scheduledDarkMode', equals: 'custom' }, description: 'HH:mm 24 小时制，如 06:00；可早于开始时间以跨过午夜。' },
  ...
  topbarClock: { label: '顶栏时钟', group: 'navigation', options: options(C.TOPBAR_CLOCK_MODES, ['关闭', '12小时制', '24小时制']) },
  topbarClockShowDate: { label: '时钟显示日期', group: 'navigation', applicableWhen: { field: 'topbarClock', in: ['12h', '24h'] } },
