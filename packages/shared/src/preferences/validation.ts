...
export const preferenceValuesSchema = z.strictObject({
  ...
  scheduledDarkMode: z.enum(C.SCHEDULED_DARK_MODES).default('off'),
  scheduledDarkStart: z.string().regex(C.SCHEDULE_TIME_PATTERN, '请使用 HH:mm 格式（如 18:00）').default('18:00'),
  scheduledDarkEnd: z.string().regex(C.SCHEDULE_TIME_PATTERN, '请使用 HH:mm 格式（如 06:00）').default('06:00'),
  ...
  topbarClock: z.enum(C.TOPBAR_CLOCK_MODES).default('off'),
  topbarClockShowDate: z.boolean().default(true),
