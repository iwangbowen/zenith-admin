...
import { createCmsEditorialTaskSchema, saveCmsFormHandlingPolicySchema, submitCmsFeedbackWorkflowSchema, updateCmsEditorialTaskSchema, updateCmsFeedbackSchema } from '../operations-validation';
...
export const cmsOperationsContract = defineContract('/api/cms/operations', {
  workspace: op.get('/workspace', { access: { permission: 'cms:dashboard:view' }, query: paginationQuery.extend({ ...siteScope, queue: queryEnum(CMS_WORKSPACE_QUEUES), keyword: keywordQuery('标题') }), response: cmsWorkspaceSchema, summary: '权限过滤的编辑与办理工作台' }),
  ...
  workflowPreview: op.get('/feedback/{id}/workflow-preview', { access: { permission: 'cms:form:list' }, params: idParam, response: workflowBusinessPreviewSchema, summary: '办理审批链路预览' }),
  ...
  tasks: op.get('/tasks', { access: { permission: 'cms:dashboard:view' }, query: paginationQuery.extend({ ...siteScope, status: queryEnum(CMS_EDITORIAL_TASK_STATUSES), ownerId: idQuery(), keyword: keywordQuery('事项标题') }), response: paginated(cmsEditorialTaskSchema), summary: '编辑事项及关联稿件发布状态' }),
